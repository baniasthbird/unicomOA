 ...
 *  @return 实例化的菜单模型
 */
+ (instancetype)ff_DropDownMenuModelWithMenuItemTitle:(NSString *)menuItemTitle menuItemCount:(NSInteger)count menuItemIconName:(NSString *)menuItemIconName menuBlock:(FFMenuBlock)menuBlock {
    FFDropDownMenuModel *model = [FFDropDownMenuModel new];
    model.menuItemTitle = menuItemTitle;
    model.menuItemCount = [NSString stringWithFormat:@"%ld",(long)count];
