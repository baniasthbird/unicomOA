...
- (id)initWithFrame:(CGRect)frame flow_num:(NSInteger)i_flow_num doc_num:(NSInteger)i_doc_num msg_num:(NSInteger)i_msg_num expansionDirection:(ExpansionDirection)direction {
    ...
    if (self) {
        [self _defaultInit];
        self.i_flow_num = i_flow_num;
        self.i_doc_num = i_doc_num;
        self.i_msg_num = i_msg_num;
...
-(void)UpdateButton:(UIButton*)btn_title num:(NSInteger)i_num {
   /*
    JSBadgeView *view_badge= [[JSBadgeView alloc]initWithParentView:btn_title alignment:JSBadgeViewAlignmentTopRight];
    view_badge.badgeText=[NSString stringWithFormat:@"%ld",(long)i_num];
    view_badge.badgePositionAdjustment=CGPointMake(-8, 10);
    view_badge.badgeTextFont=[UIFont systemFontOfSize:7];
   
    return view_badge;
    */
    if (i_num!=0) {
        CATextLayer  *_badgeLayer = [[CATextLayer alloc] init];
        _badgeLayer.string=[NSString stringWithFormat:@"%ld",(long)i_num];
        _badgeLayer.backgroundColor=[UIColor redColor].CGColor;
        _badgeLayer.fontSize=8;
        _badgeLayer.foregroundColor = [UIColor whiteColor].CGColor;
