...
@synthesize lbl_Title,lbl_Category,img_category,lbl_dapartment;
...
+(instancetype)cellWithTable:(UITableView *)tableView withCellHeight:(CGFloat)cellHeight withTitleHeight:(CGFloat)h_title withButtonHeight:(CGFloat)h_depart withTitle:(NSMutableAttributedString *)str_title withCategory:(NSString *)str_category withDepart:(NSString *)str_depart withDate:(NSString *)str_date titleFont:(CGFloat)i_titleFont otherFont:(CGFloat)i_otherFont {
    ...
    NewsInformTableViewCell *cell=[tableView dequeueReusableCellWithIdentifier:cellID];
...
-(id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier withCellHeight:(CGFloat)cellHeight withTitleHeight:(CGFloat)h_title withButtonHeight:(CGFloat)h_depart withCategory:(NSString*)str_category withTitle:(NSMutableAttributedString*)str_title withDepartment:(NSString*)str_department titleFont:(CGFloat)i_titleFont otherFont:(CGFloat)i_otherFont {
    if (self=[super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        lbl_Title=[[UILabel alloc]initWithFrame:CGRectMake(0.04*Width, 5, 0.92*Width, h_title)];
        ...
        if ([str_category isEqualToString:@"公司通知"]) {
            img_category=[[UIImageView alloc]initWithFrame:CGRectMake(0.04*Width, cellHeight-h_depart-5, 3.46875*h_depart, h_depart)];
            img_category.image=[UIImage imageNamed:@"img_inform"];
        }
