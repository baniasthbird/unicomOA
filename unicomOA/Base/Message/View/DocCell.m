...
+(instancetype)cellWithTable:(UITableView *)tableView dic:(NSDictionary *)dic_flowcontent cellHeight:(CGFloat)i_cellHeight {
    ...
    DocCell *cell=[tableView dequeueReusableCellWithIdentifier:cellID];
    
    cell=[[DocCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID dic:dic_flowcontent cellHeight:i_cellHeight];
...
-(id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier dic:(NSDictionary *)dic_flowContent cellHeight:(CGFloat)i_cellHeight{
    ...
    if (self) {
        //类别
        NSString *str_processChName=[dic_flowContent objectForKey:@"docType"];
        //公文标题
        NSString *str_processInstName=[dic_flowContent objectForKey:@"docTitle"];
        //发送人
