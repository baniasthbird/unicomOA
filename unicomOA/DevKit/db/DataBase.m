...
-(void)initTables {
    ...
    //生成备忘录表
    NSString *sqlCreateTableNotes=[NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS '%@' ('%@' INTEGER PRIMARY KEY AUTOINCREMENT, '%@' TEXT , '%@' TEXT, '%@' TEXT, '%@' TEXT, '%@' TEXT, '%@' REAL, '%@' REAL)",NOTES_TABLENAME,NOTES_ID,NOTES_FENLEI,NOTES_CONTENT,NOTES_MEETING_DATE,NOTES_DATE,NOTES_PIC_PATH,NOTES_COOR_X,NOTES_COOR_Y];
...
-(void)InsertInterFaceTable {
    ...
    if (t_array.count==0) {
        if ([_database open]) {
            ...
            //新闻列表
            NSString *insertSql11=[NSString stringWithFormat:@"INSERT INTO '%@' ('%@', '%@' ) VALUES ('%@' , '%@')",INTERFACE_TABLENAME,INTERFACE_NAME,INTERFACE_VALUE,@"NewsList",@"/default/mobile/oa/com.hnsi.erp.mobile.oa.TaskSearch.docList.biz.ext"];
            BOOL res11=[_database executeUpdate:insertSql11];
            
            //新闻详细内容
            NSString *insertSql12=[NSString stringWithFormat:@"INSERT INTO '%@' ('%@', '%@' ) VALUES ('%@' , '%@')",INTERFACE_TABLENAME,INTERFACE_NAME,INTERFACE_VALUE,@"NewsContent",@"/default/mobile/oa/com.hnsi.erp.mobile.oa.TaskSearch.docList.biz.ext"];
